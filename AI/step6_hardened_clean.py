...
# Hardened compact strict prompt: explicit format rules + forbidden patterns.
...
# Keep bad-phrase list targeted: block known format noise and forbidden sections.
_BAD_PHRASES = [
    "Instruction:",
    "Input:",
    "Solution:",
...
def _has_format_violations(raw_text: str) -> bool:
    """Detect format violations like Markdown tables, bullets, prose before sanitization."""
    patterns = [
        r"(\|\s*---|---|\d+\s*\|)",
        r"^\s*\*\s+[A-Z]",
        r"^\s*-\s+[A-Za-z]{2,}",
        r"^\d+\.\s+[A-Z][a-z]+\s+(\w+\s+)*[a-z]([.!?]|$)",
    ]
...
print("=" * 80)
print("MODEL 2 INFERENCE TEST - HARDENED COMPACT STRICT BACKLOG")
print("=" * 80)
print(f"Quick mode: {QUICK_RUN_MODE}")
print(f"Strategies: {STRATEGIES_TO_RUN}")
print(f"Token budgets: {TOKENS_BY_STRATEGY}")
print(f"Repetition penalty: 1.15 (increased from 1.12)")
...
print("Strict prompt source: hardened inline compact (with explicit format DO NOTs)")
print("Format validator: enabled (detects tables, bullets, prose)")
print("=" * 80)
...
for idx, example in enumerate(examples, start=1):
    ...
    for strategy in STRATEGIES_TO_RUN:
        ...
        has_format_issues_x = _has_format_violations(raw_x)
        ...
        strategy_name = "B: hardened strict" if strategy == "B" else "C: hardened strict"
    ...
    compliant = (
        ...
        and best_score["has_exact_end_marker"]
    )
...
if EXPORT_OUTPUT_TXT:
    ...
    lines = []
    lines.append("MODEL 2 HARDENED STRICT INFERENCE TEST")
    lines.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    lines.append(f"Dataset: {dataset_file}")
    lines.append(f"Selected rows (1-based): {selected_rows_sorted}")
    lines.append("Strict prompt: hardened inline compact with format DO NOTs")
    lines.append(f"Format validator: enabled")
    lines.append(f"Repetition penalty: 1.15")
